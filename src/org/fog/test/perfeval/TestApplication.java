...
public class TestApplication {
	...
	public static void main(String[] args) {
		...
		Log.printLine("Starting Health and Tactical Analysis System...");
		...
		try {
			...
			for(int i=0;i<idOfEndDevices.size();i++)
			{
				...
				//System.out.println("Module deadlineeeeee  "+fogDevice.getId()+fogDevice.getMips());
			}
...
//	}
	...
	private static FogDevice addEnd(String endPartialName, int userId, String appId, int parentId){
		...
		Sensor sensor = new Sensor("s-"+endPartialName, "IoTSensor", userId, appId, new DeterministicDistribution(sensingInterval)); // inter-transmission time of EEG sensor follows a deterministic distribution
		sensors.add(sensor);
	...
	private static Application createApplication(String appId, int userId){
		...
		// idOfEndDevices bir liste halinde applicationa gelecek (id ve mips'leri) 82.satirdaki gibi ---> bunlar da cihazlar
		// moduleDeadline ve moduleAddMips degerleri de gelecek. ---> bunlar istekler olacak
		
//		int i = 0;
//		for(int id:idOfEndDevices)
//		{
//			//deadlineInfo.put("mainModule", getvalue(3.00, 5.00)); // istekler icin 6 adet sure bilgisi
//			//additionalMipsInfo.put("mainModule", getvalue(0, 500)); // istekler icin 6 adet mips bilgisi
//			Map<String,Integer>fogDeviceIdMips = new HashMap<String,Integer>();
//			fogDeviceIdMips.put("clientModule", MipsOfEndDevices.get(i)); // 6 adet fog cihaz icin id - mips bilgisi
//			i++;
//			//deadlineInfo.put(id, moduleDeadline);  // modulun cihaza atanmasi islemi burada yapilmayacak.
//			//additionalMipsInfo.put(id,moduleAddMips); // ayni sekilde
//			fogIdMips.put(id,fogDeviceIdMips);
//			//System.out.println("Module deadlineeeeee  "+moduleDeadline);
//			//System.out.println("Module addmipsssssss  "+additionalMipsInfo);
//		}
		...
		//application.setFogIdMips(fogIdMips);
